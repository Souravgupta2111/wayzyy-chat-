export class Lexicons {
    static readonly invisibleScalars: Set<number> = new Set([
        ...Array.from({length: 0x200F - 0x200B + 1}, (_, i) => 0x200B + i),
        ...Array.from({length: 0x202E - 0x202A + 1}, (_, i) => 0x202A + i),
        ...Array.from({length: 0x2064 - 0x2060 + 1}, (_, i) => 0x2060 + i),
        ...Array.from({length: 0x206F - 0x206A + 1}, (_, i) => 0x206A + i),
        ...Array.from({length: 0xFE0F - 0xFE00 + 1}, (_, i) => 0xFE00 + i),
        ...Array.from({length: 0x20F0 - 0x20D0 + 1}, (_, i) => 0x20D0 + i),
        ...Array.from({length: 0xE007F - 0xE0000 + 1}, (_, i) => 0xE0000 + i),
        0x00AD, 0x034F, 0x180E, 0xFEFF, 0x20E3,
    ]);

    static readonly confusables: Record<string, string> = {
    ...
    static readonly leetToLetter: Record<string, string> = {
    ...
    static readonly letterToDigit: Record<string, string> = {
        "o": "0", "O": "0", "l": "1", "I": "1", "i": "1", "z": "2",
        "e": "3", "a": "4", "s": "5", "b": "6", "t": "7", "g": "9", "q": "9",
    };

    static readonly numberWordsCore: Record<string, string> = {
    ...
    static readonly numberWordsHomophones: Record<string, string> = {
    ...
    static readonly numberWordsFunctionWords: Record<string, string> = {
        "to": "2", "too": "2", "for": "4", "fore": "4",
        "do": "2", "no": "9",
    };

    static readonly numberWordsIndic: Record<string, string> = {
    ...
    static readonly numberWordsIndicAmbiguous: Record<string, string> = {
        "don": "2", "be": "2", "tran": "3", "shat": "7", "edu": "7", "nav": "9", "na": "9",
    };

    static readonly numberWordsRisky: Record<string, string> = {
        "nyne": "9", "ayt": "8", "fyve": "5",
    ...
    static readonly allNumberWords: Record<string, string> = {
        ...Lexicons.numberWordsCore,
        ...Lexicons.numberWordsRisky,
        ...Lexicons.numberWordsIndic,
        ...Lexicons.numberWordsIndicAmbiguous,
        ...Lexicons.numberWordsHomophones,
        ...Lexicons.numberWordsFunctionWords
    };

    static readonly hanNumerals: Record<string, string> = {
    ...
    static readonly brailleDigits: Record<string, string> = {
        "⠁": "1", "⠃": "2", "⠉": "3", "⠙": "4", "⠑": "5",
        "⠋": "6", "⠛": "7", "⠓": "8", "⠊": "9", "⠚": "0",
        "⠔": "9", "⠦": "8", "⠶": "7", "⠴": "6", "⠢": "5",
        "⠲": "4", "⠒": "3", "⠆": "2",
    ...
    static readonly romanNumerals: [string, string][] = [
        ["viii", "8"], ["vii", "7"], ["iii", "3"], ["ix", "9"], ["iv", "4"],
        ["vi", "6"], ["ii", "2"], ["xi", "11"], ["x", "10"],
        ["v", "5"], ["i", "1"],
    ];

    static regionalIndicatorLetter(scalar: number): string | null {
        if (scalar >= 0x1F1E6 && scalar <= 0x1F1FF) {
            const offset = scalar - 0x1F1E6;
            return String.fromCharCode(97 + offset);
        }
        return null;
    }

    static compatibilityFallback(scalar: number): number | null {
        const offset = (base: number, target: number) => target + (scalar - base);
        const v = scalar;
        if (v >= 0xFF10 && v <= 0xFF19) return offset(0xFF10, 0x30);
        if (v >= 0xFF21 && v <= 0xFF3A) return offset(0xFF21, 0x41);
        if (v >= 0xFF41 && v <= 0xFF5A) return offset(0xFF41, 0x61);
        if (v >= 0x1D7CE && v <= 0x1D7D7) return offset(0x1D7CE, 0x30);
        if (v >= 0x1D7D8 && v <= 0x1D7E1) return offset(0x1D7D8, 0x30);
        if (v >= 0x1D7E2 && v <= 0x1D7EB) return offset(0x1D7E2, 0x30);
        if (v >= 0x1D7EC && v <= 0x1D7F5) return offset(0x1D7EC, 0x30);
        if (v >= 0x1D7F6 && v <= 0x1D7FF) return offset(0x1D7F6, 0x30);
        if (v >= 0x1D400 && v <= 0x1D419) return offset(0x1D400, 0x41);
        if (v >= 0x1D41A && v <= 0x1D433) return offset(0x1D41A, 0x61);
        if (v >= 0x1D670 && v <= 0x1D689) return offset(0x1D670, 0x41);
        if (v >= 0x1D68A && v <= 0x1D6A3) return offset(0x1D68A, 0x61);
        if (v >= 0x2460 && v <= 0x2468) return 0x31 + (v - 0x2460);
        if (v === 0x24EA || v === 0x24FF) return 0x30;
        if (v === 0x2070) return 0x30;
        if (v === 0x00B9) return 0x31;
        if (v === 0x00B2) return 0x32;
        if (v === 0x00B3) return 0x33;
        if (v >= 0x2074 && v <= 0x2079) return 0x34 + (v - 0x2074);
        if (v >= 0x2080 && v <= 0x2089) return 0x30 + (v - 0x2080);
        return null;
    }

    static readonly repeatModifiers: Record<string, number> = {
        "double": 2, "dubble": 2, "twice": 2, "triple": 3, "treble": 3,
        "tripple": 3, "quad": 4, "quadruple": 4,
    };

    static readonly natoAlphabet: Record<string, string> = {
    ...
    static readonly separatorWords: Record<string, string> = {
    ...
    static readonly solicitationCues: string[] = [
    ...
    static readonly steeringCues: string[] = [
    ...
    static readonly pretextCues: string[] = [
    ...
    static readonly promptInjectionCues: string[] = [
    ...
    static readonly morseToChar: Record<string, string> = {
        ".-": "a", "-...": "b", "-.-.": "c", "-..": "d", ".": "e", "..-.": "f",
        "--.": "g", "....": "h", "..": "i", ".---": "j", "-.-": "k", ".-..": "l",
        "--": "m", "-.": "n", "---": "o", ".--.": "p", "--.-": "q", ".-.": "r",
        "...": "s", "-": "t", "..-": "u", "...-": "v", ".--": "w", "-..-": "x",
    ...
    static readonly platformsStrong: Set<string> = new Set([
    ...
    static readonly platformsWeak: Set<string> = new Set([
        "wa", "ig", "tg", "fb", "dm", "pm", "sc", "wapp", "wats", "gram",
    ]);

    static readonly platformsStripped: Set<string> = new Set([
        "whtspp", "whtsp", "wtspp", "wtsap", "instgrm", "instgm", "nsta",
        "tlgrm", "tlgm", "snpcht", "snpct", "sgnl", "vbr", "wcht", "dscrd",
        "fcbk", "mssngr", "gml", "ymail",
    ]);

    static readonly platformsCollapsed: Set<string> = new Set(
        Array.from(Lexicons.platformsStrong).map(p => p.replace(/(.)\1+/g, '$1'))
    );

    // Stubs for missing attributes
    static selfHarmPhrases: string[] = [
        "kill myself", "end my life", "want to die", "suicide", "suicidal",
        "no reason to live", "end it all", "hurt myself", "harm myself",
    ];
    static threatPhrases: string[] = [
    ...
    static harassmentPhrases: string[] = [
    ...
    static discriminationPhrases: string[] = [];
    static readonly personTargets: Set<string> = new Set([
    ...
    static readonly propertyTargets: Set<string> = new Set([
    ...
    static readonly devanagariPersonStems: string[] = [
        "तुम",
        "तुझ",
        "तेर",
        "तू",
    ...
    static cyrillicPersonTokens: Set<string> = new Set([
    ...
    static readonly nativeScriptConditionalCues: string[] = [
        "नहीं तो", "नही तो", "वरना", "वर्ना", "अन्यथा",
        "иначе", "или я", "если не", "в противном случае",
    ];
    static readonly commonTLDs: Set<string> = new Set([
    ...
    static readonly protocolHints: string[] = [
    ...
    static readonly safeDomains: Set<string> = new Set();
    static readonly upiSuffixes: Set<string> = new Set([
    ...
    static readonly identifierStoplist: Set<string> = new Set([
    ...
    static readonly contactIntent: string[] = [
    ...
    static readonly offPlatformIntent: string[] = [
    ...
    static readonly paymentKeywords: Set<string> = new Set([
    ...
    static readonly shorteners: Set<string> = new Set([
    ...
    static slurTerms: Set<string> = new Set();

    static readonly profanityStrong: Set<string> = new Set([
    ...
    static readonly genericWordPlatforms: Set<string> = new Set([
        "line", "meet", "signal", "zoom", "band", "chat", "talk", "duo",
        "join", "session", "circle", "path", "hike", "tango", "marco",
    ]);

    private static readonly fuzzyStoplistData: Set<string> = new Set([
    ...
    static get fuzzyStoplist(): Set<string> { return this.fuzzyStoplistData; }

    static editDistance(a: string, b: string, cap: number = 2): number {
        const s = Array.from(a), t = Array.from(b);
    ...
    static fuzzyPlatform(token: string): string | null {
        if (token.length < 4 || token.length > 16 || !/^[a-zA-Z]+$/.test(token)) return null;
        const lower = token.toLowerCase();
        if (this.platformsStrong.has(lower)) return lower;
        if (this.platformFuzzyStoplist.has(lower)) return null;
        for (const name of this.platformsStrong) {
            if (name.length < 5) continue;
            if (Math.abs(name.length - lower.length) > 1) continue;
            if (this.editDistance(lower, name, 1) <= 1) return name;
        }
        return null;
    }

    static fuzzyNumberWord(token: string): string | null {
        const exact = this.numberWordsCore[token.toLowerCase()];
        if (exact !== undefined) return exact;
        const lower = token.toLowerCase();
        if (lower.length < 3 || lower.length > 9 || !/^[a-zA-Z]+$/.test(lower)) return null;
        if (this.fuzzyStoplist.has(lower)) return null;
        if (this.numberWordsHomophones[lower] !== undefined ||
            this.numberWordsFunctionWords[lower] !== undefined ||
            this.numberWordsRisky[lower] !== undefined ||
            this.numberWordsIndic[lower] !== undefined ||
            this.numberWordsIndicAmbiguous[lower] !== undefined) return null;
        for (const [word, digit] of Object.entries(this.numberWordsCore)) {
            if (Math.abs(word.length - lower.length) > 1) continue;
            if (this.editDistance(lower, word, 1) <= 1) return digit;
            if (word.length === lower.length && this.isTransposition(lower, word)) return digit;
        }
        return null;
    }

    static isTransposition(a: string, b: string): boolean {
        const x = Array.from(a), y = Array.from(b);
        if (x.length !== y.length || x.length < 3) return false;
        const diffs: number[] = [];
        for (let i = 0; i < x.length; i++) {
            if (x[i] !== y[i]) diffs.push(i);
        }
        if (diffs.length !== 2 || diffs[1] !== diffs[0]! + 1) return false;
        return x[diffs[0]] === y[diffs[1]] && x[diffs[1]] === y[diffs[0]];
    }

    static collapseRuns(s: string): string {
        let out = "";
    ...
    static compatibilityFallbackString(codePoint: number): string | null {
        if (codePoint >= 0x2474 && codePoint <= 0x247C) return `(${codePoint - 0x2474 + 1})`;
        if (codePoint >= 0x247D && codePoint <= 0x2487) return `(${codePoint - 0x247D + 10})`;
        return null;
    }

    static looksLikeHandle(token: string): boolean {
        if (token.length < 4 || token.length > 30) return false;
        const hasDigit = /[0-9]/.test(token);
        const hasSep = token.includes("_") || token.includes(".");
        if (!hasDigit && !hasSep) return false;
        return Array.from(token).every(c => /\p{L}|\p{N}/u.test(c) || c === "_" || c === "." || c === "-");
    }
}
